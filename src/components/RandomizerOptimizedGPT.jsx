    ...
    setLoadout((prevLoadout) => {
      ...
      return {
        ...
        stratagems: prevLoadout.stratagems.map((strat, index) => {
          // if current stratagem is locked, filter it out of allStratagems
          locks.stratagems[index] &&
            (allStratagems = allStratagems.filter(
              (curStrat) => curStrat.name !== strat.name
            ));
    ...
    <Container>
      <div className="d-flex align-items-center flex-column vh-85">
        ...
        <Button
          ...
          onClick={() => {
            runMultipleTimes(randomizeLoadout, 20, 50);
          }}
